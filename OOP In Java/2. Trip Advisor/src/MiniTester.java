...
public class MiniTester {
	...
	public int testRoom_2(int testIdx) {
		...
		try {
			...
			if (r2 == found)
				...
				comment = comment
						+ "Error: findAvailableRoom doesn't find the first available room if multiples of that type exist.\n";
	...
	public int testRoom_3(int testIdx) {
		String comment = "[" + testIdx
				+ "]: Test whether makeRoomAvailable changes the FIRST unavailable to available.\n";
		int maxScore = 1;
		int grade = 0;
		...
		try {
			...
			if (res == true && getAvailability(r3) == true)
				grade += 1;
			else {
				comment = comment
						+ "Error: makeRoomAvailable didn't change the right room's availablility or it didn't return true.\n";
			}
	...
	public int testAirport_1(int testIdx) {
		...
		try {
			...
			if (distance2 == 6)
				...
				comment = comment
						+ "Error: getDistance doesn't return the right value when Airports have different coordinates.\n";
			}
	...
	public int testHotel_1(int testIdx) {
		...
		try {
			...
			if (getAvailability(r1) == true && price == 9000 && getAvailability(studentRoom) == false)
				grade += 3; // shouldn't have updated the original room since a deep copy
			else {
				comment = comment
			...
			if (res1 == true && getAvailability(studentRoom) == true)
				grade += 3;
			else {
	...
	public int testHotelReservation_0(int testIdx) {
		...
		try {
			...
			if (grade < 1)
				comment = comment
						+ "Error: making a HotelReservation actually should reserve the room (availability changes).\n";
	...
	public int testHotelReservation_2(int testIdx) {
		...
		try {
			...
			if (res == false)
				...
				comment = comment
						+ "Error: equals in HotelReservation returns true when both FlightReservations but not equal.\n";
			}
	...
	public int testBasket_0(int testIdx) {
		...
		try {
			...
			if (r2.equals(myReservations[1]) && r1.equals(myReservations[0]))
				grade += 2;
			else {
	...
	public int testBasket_2(int testIdx) {
		String comment = "[" + testIdx
				+ "]: Test that remove method can remove and shift list if such a reservation exists.\n";
		int maxScore = 6;
		int grade = 0;
	...
	public int testCustomer_2(int testIdx) {
		...
		try {
			...
			if (c1.getBasket().getNumOfReservations() == 2)
				grade += 2;
			else {
